...
async function generateValidStreetViewLocation() {
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      // 2. Use Street View Metadata API to verify location validity
      ...
      // If no street view found, continue to next iteration
      ...
      // Check if Geocoding API returned results successfully
      if (geoResponse.data.status === 'OK' && geoResponse.data.results.length > 0) {
        ...
        if (countryComponent) {
          countryCode = countryComponent.short_name; // 'short_name' is typically 2-letter country code like 'US'
        }
      }
      ...
      // 4. Apply reselection logic based on country code to maintain regional balance
      ...
      // 5. If all checks passed, return final result
      ...
      // Error handling to avoid interrupting process due to network issues
      if (error.response) {
